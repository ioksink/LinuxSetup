---
title: "Sync Zotero in Your Own Server"
date: 2026-07-06
draft: false
---
...
- Arch-based distro runs `sudo pacman -i <packageName>`. There is also [an ArchWiki page for `rsync`](https://wiki.archlinux.org/title/Rsync).
- Windows users cannot run `rsync` natively, so the best way to achieve the same result is using a tool called **FreeFileSync**. It is free, open-source, visual (GUI), and does exactly what `rsync` does but with buttons and colours.

**WINDOWS USER? DIRECTLY JUMP TO [HERE](#for-windows-users)!**
...
1. Will it overwrite your GitHub key?
...
2. The Best Practice: Create a "Named Key"

Since you are using this computer for multiple different things (GitHub, University Server, etc.), the professional way to do this is to give each key a **unique name**, e.g. id_university. Instead of pressing Enter, give the key a specific name for the university. Run this:

```bash
ssh-keygen -t ed25519 -f ~/.ssh/id_university
```
*   `-t ed25519`: Sets the secure algorithm. The modern standard. It is much smaller, faster, and more secure than RSA.
*   `-f ~/.ssh/id_university`: Tells the computer to save the key as `id_university` instead of the default `id_ed25519`. Also fine to change the name to anything you like.

**Now you have two separate keys:**
1. `id_ed25519` $\rightarrow$ for GitHub.
2. `id_university` $\rightarrow$ for your Zotero server.

### 3. How to use the Named Key
...
**1. On your working desktop:** Upload everything you have to the server.
```bash
rsync -avz --no-perms --no-owner --no-group ~/Zotero/storage/ "<UserID>@login.uni-freiburg.de:/home/<UserID>/<storageFolder>"
```
*Now the server has everything from your Desktop.*

**2. On your personal laptop:** Upload everything you have to the server. **CRITICAL: Remove the `--delete` flag for this step.**
```bash
rsync -avz --no-perms --no-owner --no-group ~/Zotero/storage/ "<UserID>@login.uni-freiburg.de:/home/<UserID>/<storageFolder>"
```
...
**1. On your working desktop:** Pull everything from the server. (Put `--delete` back in to remove any duplicates/old versions).
```bash
rsync -avz --no-perms --no-owner --no-group --delete "<UserID>@login.uni-freiburg.de:/home/<UserID>/<storageFolder>" ~/Zotero/storage/
```

**2. On your personal laptop:** Pull everything from the server.
```bash
rsync -avz --no-perms --no-owner --no-group --delete "<UserID>@login.uni-freiburg.de:/home/<UserID>/<storageFolder>" ~/Zotero/storage/
```

### Maintain the "Common" State
Now that you are integrated, "pull" before working and "push" after working should also be done as daily routine.

**The Workflow moving forward:**
1. **Start of Session:** Always `rsync` **FROM** Server $\rightarrow$ **TO** Local. (Pull)
2. **End of Session:** Always `rsync` **FROM** Local $\rightarrow$ **TO** Server. (Push)
